...
function addRecipe() {
  ...
  const type = prompt(
    "Enter the type of recipe (e.g., main course, dessert, appetizer):"
  );
...
function updateRecipe() {
  ...
  if (recipe) {
    const newName = prompt(
      `Enter new name (current: ${recipe.name}):`,
      recipe.name
    );
    const newType = prompt(
      `Enter new type (current: ${recipe.type}):`,
      recipe.type
    );
    const newDescription = prompt(
      `Enter new description (current: ${recipe.description}):`,
      recipe.description
    );
